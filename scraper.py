...
def get_article(url):
    ...
    Returns:
        dict: Структурированные данные с полями:
            - url: исходный URL
...
def _fetch_page(url):
    ...
    Returns:
        tuple: (BeautifulSoup объект, None) или (None, dict с ошибкой)
    """
...
def _truncate_content(text, max_length=MAX_CONTENT_LENGTH):
...
def _parse_github(url):
    ...
    repo = repo.rstrip('/')  # Убираем trailing slash если есть
    ...
    # README — ищем отрендеренный markdown
...
def _extract_github_language(soup):
    """Извлекает основной язык программирования."""
    lang_elem = soup.find('span', class_='color-fg-default', attrs={'itemprop': 'programmingLanguage'})
    if lang_elem:
        return lang_elem.get_text(strip=True)
    return 'Не определён'    

def _extract_github_readme(soup):
...
# Алиас для обратной совместимости с pipeline.py
get_structured_habr_article = _parse_habr
